...
import { aplanarPaginas, useComunicadosFeed } from '../hooks/useComunicadosFeed';
import { ComunicadoCard } from './ComunicadoCard';

export function ComunicadosList() {
  const query = useComunicadosFeed();
  ...
  if (query.isPending) {
    return (
      <EstadoCentrado>
        <ThemedText tone="secondary">Cargando comunicados...</ThemedText>
      </EstadoCentrado>
    );
  }
  ...
  if (query.isError) {
    return (
      <EstadoCentrado gap>
        <ThemedText tone="secondary">No se pudo cargar los comunicados.</ThemedText>
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }: ListRenderItemInfo<Comunicado>) => (
        <ComunicadoCard comunicado={item} />
      )}
      ...
      ListEmptyComponent={
        <EstadoCentrado>
          ...
          <ThemedText tone="secondary">
            Cuando se publiquen comunicados para tu audiencia, aparecerán aquí.
          </ThemedText>
        </EstadoCentrado>
      }
